-- Fix ambiguous PL/pgSQL column references in public.unified_search.
  ...
  with candidates as (
    ...
    select
      ...
      public.normalize_unified_search_text(
        concat_ws(' ', title_text, content_text, coalesce(candidates.due_date::text, ''))
    ...
    select
      ...
      case
        ...
        when normalized.due_date::text = v_query then 40
        else 0
