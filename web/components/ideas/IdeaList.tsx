...
export default function IdeaList({ ideas, onChanged, onDeleted }: { ideas: Idea[]; onChanged: (idea: Idea) => void; onDeleted: (id: string) => void }) {
  ...
  return <div className="space-y-3">
    ...
    {ideas.map((idea) => <article className="rounded-box border border-base-300 bg-base-100 p-4" key={idea.id}>
      {editingId === idea.id ? <div className="space-y-3"><textarea className="textarea textarea-bordered w-full" value={draft} onChange={(event) => setDraft(event.target.value)} maxLength={4000} /><div className="flex gap-2"><LoadingButton className="btn btn-primary btn-sm" onAction={() => { const clean = draft.trim(); if (clean) return update(idea, { content: clean }); setError("Idea content is required."); return Promise.reject(new Error("Idea content is required.")) }} pendingLabel="Saving..." onError={() => setError("Idea content is required.")} disabled={busyId === idea.id}>Save</LoadingButton><button className="btn btn-ghost btn-sm" type="button" onClick={() => setEditingId(null)} disabled={busyId === idea.id}>Cancel</button></div></div> : <>
