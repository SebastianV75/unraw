...
export type CaptureIdea = {
	content: string;
	area_id: string | null;
	suggested_new_area: string | null;
};
export type CaptureSecondBrain = {
	title: string;
	content: string;
	area_id: string | null;
	tags: string[];
...
export type CaptureOutput = {
	tasks: CaptureTask[];
	ideas: CaptureIdea[];
	second_brain: CaptureSecondBrain[];
	suggestions: CaptureSuggestion[];
};
export type CapturePreference = "tasks" | "ideas" | "knowledge";
export type CaptureAssignments = Record<string, string | null>;
export type CaptureSnapshot = CaptureOutput & {
...
export type CaptureHistory = {
	...
	output_snapshot: CaptureSnapshot;
	saved_references: SaveCaptureResult;
